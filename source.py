...
def search_pdfs(page_num, directory=search_path):
...
def merge_page_with_pdfs(page_num):
    ...
    if not (isinstance(matching_files, list) and matching_files):
        ...
        return f"aucun fichier trouvé pour ce pattern {the_type}_{intmed}_{police}_*.pdf"
    ...
    for pdf_file in matching_files:
        pdf_path = os.path.join(search_path, pdf_file)#construire le chemin d'accès au fichier en joignant le dossier "PATH_SEARCH" avec le nom de fichier "pdf_file."
    ...
    output_pdf = os.path.join(merged_pdf_directory, f"{the_type}_{intmed}_{police}.pdf")
